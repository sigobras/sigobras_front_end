...
const ReportTable: React.FC = () => {
	...
	function getFormattedData(colLabel: string, format: string, indice: number) {
		...
		const calculations: CalculationFunctions = {
			...
			total_cemento: () =>
				Number(Datos[indice].cantidad_cemento) +
				Number(Datos[indice].cantidad_vencidos_cemento),
			saldo: () =>
				Number(Datos[indice].pim) - Number(Datos[indice].total_certificado),
	...
	function agregarFormatoNumero(numero: any) {
		...
		let [signo, parteEntera, parteDecimal] = numero
			.toString()
			.match(/^(-?)([\d,.]+)(?:\.(\d{2}))?$/)
			.slice(1);
	...
	return (
		<>
			<Button className={classes.button} onClick={handleAddRow} />
			<TableContainer>
				<Table>
					<TableHead>
						<TableRow>
					...
					<TableBody className={classes.tableBody}>
						{Datos.map((row, index) => (
							<TableRow key={row.id}>
								<TableCell className={classes.disabled}>{index + 1}</TableCell>
