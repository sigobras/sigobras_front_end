...
class GestionTareas extends Component {
	...
	demoFuncion(user) {
		...
		const users = this.state.PositsFiltrado;
	...
	AgregaTarea(e) {
		...
		//     return
		// }
		const funcionMap = data => [data.id_acceso];
		const Personal = InputPersonal.map(funcionMap);
		let idTarea = PositsFiltrado.id_tarea;
	...
	subtareaAdd(idTarea) {
		...
		axios
			...
			.then(res => {
				...
				const dataRes = this.state.DatSubordinadospi;
	...
	onChangeImgMetrado(e) {
		const inputValueImg = e.target.files[0];
		...
		if (
			...
			const url = URL.createObjectURL(inputValueImg);
			// console.log("url", url);
	...
	DescargarArchivo(data) {
		const tipoArchivo = Extension(data);
	...
	textPorcentEdit(e, indexT, id_tarea) {
		...
		axios
			...
			.then(res => {
				...
				const tareasRecibidas = this.state.DataTareasApi;
	...
	GuardaComentario(e) {
		e.preventDefault();
		const valueInputComentario = e.target[0].value;
	...
	onDrop(ev, cat, idAccesso) {
		...
		const { indexSubTarea, DatSubordinadospi, DataTareasEmitidosApi } =
			this.state;
		const id = ev.dataTransfer.getData('id');
		const DataSubordinados = DatSubordinadospi;
		const DataEmitidos = DataTareasEmitidosApi;

		const fiterIdacceso = DataSubordinados.map(e => {
			return e.id_acceso;
		}).indexOf(idAccesso);
															...
															<div className='text-center text-white'>
																{PositsFiltrado.diasTranscurridos < 0 ? (
																	<i>
																		...
																		tienes {PositsFiltrado.diasTotal} dias para
																		Concluirlo.
																	</i>
																) : (
																	<i>
							...
							<div className='ModalContaninerMas'>
								<div className='bg-info p-2'>
									<div className='d-flex flex-column'>
										<div className='text-uppercase text-center h6'>
											{DataMostrarDetallesApi.cargo}
										</div>
										<div className=''>
											<img
