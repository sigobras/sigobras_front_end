...
export default ({ data, AnyoSeleccionado }) => {
	...
	async function cargarData() {
		const res = await axios.get(`${UrlServer}/v1/avance/resumenAnual`, {
			params: {
				id_ficha: data.id_ficha,
				anyo: AnyoSeleccionado,
			},
		...
		setData(res.data);
		const total = res.data.reduce((acc, item) => acc + item.fisico_monto, 0);
		setDataTotal(total);
	}
			...
			>
				<tbody>
					...
					{(() => {
						...
						for (let i = 1; i <= 6; i++) {
							const mes1 = buscarMes(i) || {
								fisico_monto: 0,
								fisico_programado_monto: 0,
							};
							const mes2 = buscarMes(i + 6) || {
								fisico_monto: 0,
								fisico_programado_monto: 0,
							};
							dataTemp.push(
