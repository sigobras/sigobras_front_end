...
export default ({ data, AnyoSeleccionado, setMensajeGuardando }) => {
	...
	async function cargarData() {
		setLoading(true);
		const res = await axios.get(`${UrlServer}/v1/datosAnuales`, {
			params: {
				id_ficha: data.id_ficha,
				anyo: AnyoSeleccionado - 1,
			},
		});
		const res2 = await axios.get(`${UrlServer}/v1/datosAnuales`, {
			params: {
				id_ficha: data.id_ficha,
	...
	async function cargarAvanceFinancieroAcumulado() {
		const res = await axios.get(`${UrlServer}/v1/avance/acumuladoAnual`, {
			params: {
				id_ficha: data.id_ficha,
				anyo: AnyoSeleccionado - 1,
			},
