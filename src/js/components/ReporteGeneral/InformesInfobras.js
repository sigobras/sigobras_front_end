...
export default ({ data, recargar, AnyoSeleccionado }) => {
	...
	function buscarMesInData(mes) {
		const tempItem = Informes.find((item, i) => item.mes == mes);
		return tempItem;
	}
	// render
	...
	async function cargarInformesUbicaciones() {
		try {
			const res = await axios.get(`${UrlServer}/v1/informesUbicaciones`);
...
function NuevoInforme({
	...
	async function guardarData(e) {
		...
		try {
			...
			for (const key in InputObjectModificada) {
...
function DescripcionInforme({ id }) {
	...
	function handleInputChange(event, index, item) {
		const flagClone = [...FlagCambios];
		flagClone[item.id] = true;
		setFlagCambios(flagClone);
		const clone = [...Descripcion];
				...
				<tbody>
					{Descripcion.map((item, i) => [
						...
						<DescripcionDetalles
							...
							index={i + 1}
							ref={ref => {
								const clone = RefDescripcionInforme;
								clone[item.id] = ref;
								// clone[1] = ref;
