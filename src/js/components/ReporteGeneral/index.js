...
export default ({ recargar }) => {
	...
	const [ListInterfacesDefecto, setListInterfacesDefecto] =
		useState(interfaces);
	const [ListOrders, setListOrders] = useState([]);
	const [ListOrdersCargado, setListOrdersCargado] = useState(false);
	async function cargarListOrders() {
		try {
			const res = await axios.get(`${UrlServer}/v1/reporteGeneral/interfaces`, {
				params: {
	...
	function ordernarInterfaces() {
		const newOrder = [];
		const listIndexUsados = [];
		const indexCalculado = ListOrders.findIndex(
		...
		const Order = ListOrders[indexCalculado].orden;
		for (let index = 0; index < Order.length; index++) {
			const element = Order[index];
			const newItem = ListInterfacesDefecto[element.posicion];
	...
	async function cargarObras() {
		...
		const res = await axios.get(`${UrlServer}/v1/obras/resumen`, {
			params: {
				id_acceso: sessionStorage.getItem('idacceso'),
	...
	function calcularNuevoOrden() {
		...
		for (let index = 0; index < ListInterfaces.length; index++) {
			...
			const indexFound = ListInterfacesDefecto.findIndex(
	...
	async function actualizarOrdenInterfaces() {
		try {
			...
			const res = await axios.put(`${UrlServer}/v1/reporteGeneral/interfaces`, [
				{
			...
			{ListInterfaces.map((item, i) => (
				<span
					...
					onDrop={e => {
						...
						const tempArray = [...ListInterfaces];
						if (indexOrigen < i) {
							i--;
						}
