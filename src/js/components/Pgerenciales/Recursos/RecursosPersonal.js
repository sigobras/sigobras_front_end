...
class RecursosPersonal extends Component {
	...
	getanyos(id_ficha) {
		...
		axios
			.post(
				`${UrlServer}/SelectRecPersonalAnyos`, // RUTA DEL API
				{
					id_ficha,
				},
			)
	...
	getmeses(anyo) {
		...
		axios
			.post(
				`${UrlServer}/SelectRecPersonalMeses`, // RUTA DEL API
				{
	...
	getsemanas(mes) {
		...
		axios
			.post(
				`${UrlServer}/SelectRecPersonalSemana`, // RUTA DEL API
				{
					id_ficha: this.state.cajaidficha,
	...
	getGastoMO(semana) {
		...
		axios
			.post(
				`${UrlServer}/GastoManodeObra`, // RUTA DEL API
				{
					id_ficha: this.state.cajaidficha,
					anyo: this.state.anyoseleccionado,
			...
			.then(async respuesta => {
				...
				let index = 0;
				const partidasgastoreales = [];
				let gastoexptec_mo = 0;
				...
				for (let i = 0; i < respuesta.data.PartidasSemanal.length; i++) {
					...
					const cantidadPersonal = respuesta.data.cantidadPersonal[index];

					if (element.id_partida == cantidadPersonal.id_partida) {
						Object.assign(
							respuesta.data.PartidasSemanal[i],
							respuesta.data.cantidadPersonal[index],
						);
						index++;
					}
					...
					for (let j = 0; j < respuesta.data.fechasavance.length; j++) {
						...
						const gastoreal = element.parcial_mo * element['metrado' + j];
	...
	addcargo() {
		const cargotemp = this.state.CantidadPersonalReal;
		cargotemp.push({
	...
	updateinput(target, index) {
		...
		const CantidadPersonalReal = this.state.CantidadPersonalReal;
	...
	guardardata_mo() {
		...
		if (confirm('Está seguro de guardar la información?')) {
			axios
				.post(
					`${UrlServer}/putresumengastomo`, // RUTA DEL API
	...
	getcargos_obreros() {
		axios
			.get(
				`${UrlServer}/cargos_obreros`, // RUTA DEL API
			)
			.then(respuesta => {
	...
	get_historial_gasto_mo() {
		axios
			.post(
				`${UrlServer}/historial_gasto_mo`, // RUTA DEL API
	...
	async updateinputCostoReal(target, index) {
		...
		const CostoRealMO = this.state.CostoRealMO;
	...
	sumatoriacostodia() {
		...
		for (let i = 0; i < this.state.CantidadPersonalReal.length; i++) {
			...
			const costodia = element.cantidad * element.costogore * element.dias;
			sumatoria += costodia;
		}
	...
	get_chart_data_mo() {
		axios
			.post(
				`${UrlServer}/chart_mo_dia`, // RUTA DEL API
			...
			.then(respuesta => {
				...
				const gastoestimadoexptec_mo = [];
				const gasto_dia_gore = [];
				const fechas_avance_mes = [];
				for (let i = 0; i < respuesta.data.gastoestimadoexptec_mo.length; i++) {
					...
					gasto_dia_gore.push(
						Redondea1(respuesta.data.gasto_dia_gore[i].gasto_gore),
					);
	...
	async get_perdida_ganancia() {
		let gastoestimadoexptec_mo_semana = [];
		await axios
			.post(
				`${UrlServer}/gastoestimadoexptec_mo_semana`, // RUTA DEL API
		...
		axios
			.post(
				`${UrlServer}/gastoestimadogore_mo_semana`, // RUTA DEL API
		...
		return (
			<div>
				<nav>
					<select onChange={event => this.getmeses(event.target.value)}>
						...
						{this.state.anyos.map((element, index) => (
							<option key={index} value={element.anyo}>
								{element.anyo}
							</option>
						))}
					</select>
					...
					<select onChange={event => this.getsemanas(event.target.value)}>
						...
						{this.state.meses.map((element, index) => (
							<option key={index} value={element.mes}>
								{element.mes}
							</option>
						))}
					</select>
					...
					<select onChange={event => this.getGastoMO(event.target.value)}>
						...
						{this.state.semanas.map((element, index) => (
							<option key={index} value={element.semana}>
								{element.semana}
							</option>
						))}
					</select>
				...
				<table className='tabla2'>
					<thead>
						<tr>
							...
							{this.state.gastoestimadoexptec_mo_semana.map(
								(element, index) => [
					...
					<tbody>
								...
								(element, index) => [
									...
									<td rowSpan='3'>
										{Redondea(
											(1 -
												element.valor /
													this.state.gastoestimadogore_mo_semana[index]
														.gasto_semana_mo) *
												100,
										)}{' '}
										%
						...
						<tr>
							...
							{this.state.gastoestimadogore_mo_semana.map((element, index) => [
								<td>
									{Redondea(
										this.state.gastoestimadoexptec_mo_semana[index].valor -
											element.gasto_semana_mo,
									)}
								</td>,
							])}
						</tr>
				...
				<table className='tabla1 table-responsive table-hover'>
					<thead>
						<tr>
							<td
								className='detalle1'
								colSpan={this.state.cargosPersonal.length + 7}
							>
								SISTEMA DE INFORMACIÓN GERENCIAL DE OBRAS
							</td>
							{this.state.fechasavance.map((element, index) => (
								<td className='resultadomo' colSpan='4' rowSpan='2'>
									{this.getnumerodia(element.fecha)}
								</td>
							))}
						</tr>
						<tr>
							...
							<td className='partida' rowSpan='3'>
								DESCRIPCION DE PARTIDAS EJECUTADAS
							</td>
							<td rowSpan='3'>UND</td>
							<td rowSpan='3'>REND</td>
							<td rowSpan='3'>P/U</td>
							<td colSpan={this.state.cargosPersonal.length + 1}>
								PERSONAL SEGÚN EXPEDIENTE TÉCNICO
							</td>
						</tr>
						<tr>
						...
						{this.state.gastosMO.map((element, index) => (
							<tr>
								...
								{this.state.fechasavance.map((element2, index2) => [
									...
									<td>
										{Redondea(
											element['metrado' + index2] * element.costo_unitario,
										)}{' '}
									</td>,
									<td>
										{Redondea(
											(element['metrado' + index2] / element.rendimiento) * 100,
										)}
									</td>,
									<td className='resultadomo'>
										{Redondea(element['metrado' + index2] * element.parcial_mo)}
									</td>,
								])}
							</tr>
						))}
				...
				<table className='tabla1'>
						...
						{this.state.CantidadPersonalReal.map((element, index) => (
							<tr>
								...
								<td>
									...
									>
										{this.state.cargos.map((element2, index2) => (
											<option value={element2.idcargos_obreros}>
												{element2.nombre}
											</option>
										))}
									</select>
								...
								<td>
									S/.{' '}
									{Redondea(
										element.cantidad * element.costogore * element.dias,
									)}
								</td>
							</tr>
						))}
					...
					<tfoot>
						...
						<button
							className='botoncomp'
							disabled={this.state.semanaseleccionada != 0 ? '' : 'disabled'}
							onClick={event => this.guardardata_mo()}
						>
							Guardar
						</button>
					</tfoot>
				...
				<table className='tabla1 table-hover table-responsive'>
					<thead>
						<tr>
							<td colSpan={this.state.fechasavance.length + 1}>
								{' '}
								RESUMEN DE GASTO DE LA SEMANA EN MANO DE OBRA
							</td>
						</tr>
						<tr>
							<td>DETALLE</td>
					...
					<tbody>
						...
						<tr>
							...
							{this.state.fechasavance.map((element, index) => (
								<td>
									{Redondea(
										this.sumatoriacostodia() / this.state.fechasavance.length,
									)}
								</td>
							))}
						</tr>
				...
				<table className='tabla2'>
					...
					<tbody>
						<tr>
							...
							<td rowSpan='3'>
								{Redondea(
									(1 - this.state.gastoexptec_mo / this.sumatoriacostodia()) *
										100,
								)}{' '}
								%
							</td>
						...
						<tr>
							<td className='resultadomo'>(A - B) PÉRDIDA Y/O GANANCIA </td>
							<td className='resultadomo'>
								{Redondea(this.state.gastoexptec_mo - this.sumatoriacostodia())}
							</td>
						</tr>
					</tbody>
				</table>
