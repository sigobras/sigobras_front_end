...
class Proyeccion extends Component {
	...
	selectObra(idObra) {
		// (EN SELECT OBRA FUNCION SE PUEDE TRABAJAR CON CUALQUIER NOMBRE)
		this.setState({
		...
		axios
			.post(
				`${UrlServer}/AnyoInicial`, // RUTA DEL API
				{
					id_ficha: idObra,
				},
			)
			.then(respuesta => {
				// respuesta del api año inicial
				const listaanyos = [respuesta.data.anyo_inicial];
				for (let i = 1; i < 11; i++) {
				...
				this.setState({
					// sirve para modificar el valor de una variable
	...
	getComponentes(anyo) {
		...
		axios
			.post(
				`${UrlServer}/ComponentesProyeccion`, // RUTA DEL API
				{
	...
	async capturaInputProyeccionExp(evento, mes, index_comp) {
		evento.persist();
		const value_temp = parseFloat(evento.target.value) || 0;
		const componentes_temp = this.state.componentes;
		...
		const componentecopia = this.state.cajalistacomponente;
	...
	async guardarProyeccionExp(id_componente, mes) {
		...
		await axios
			...
			.then(respuesta => {
				// respuesta del api año inicial   //"data" es propio detransferencia de la datos
			})
			.catch(error => {});
	...
	async capturaInputProyeccionVar(evento, mes, index_comp) {
		evento.persist();
		const value_temp = parseFloat(evento.target.value) || 0; // parseFloat se convierte el texto a numero
		const componentes_temp = this.state.componentes;
	...
	async guardarProyeccionVar(id_componente, mes) {
		...
		await axios
			.post(
				`${UrlServer}/postProyeccionVar`, // ruta del api
	...
	get_chart_data(id_componente) {
		axios
			.post(
				`${UrlServer}/chartproyeccion`, // RUTA DEL API
				{
					id_ficha: this.state.cajaidficha,
	...
	activarEdicion(index) {
		// index de componentes - solo se puede mandar los datos que esten en la interfaz dentro de la tabla, son datos que se pueden pasar de la tabla hacia esta funcion activarrEdicion
		const componenteTemp = this.state.componentes[index];
		// preparación del componente para formato lista
		const componenteslista = [
			// [
	...
	render() {
		// es todo lo que se imprime en la pantalla render es una funcion del propio react
		const { cajaanyo } = this.state; // SE USO PARA PONER EL AÑO EN EL CHART TITULO
		...
		return (
			...
			<div className='container-fluid'>
				<select
					className='selectproye'
					onChange={event => this.getComponentes(event.target.value)}
				>
					<option>Seleccione el año</option>
					{this.state.anyosproyectados.map(
						(
							anyo,
							index, // HACE INTERACCION CON UN SOLO ELEMENTO
						) => (
							...
							<tbody key={index}>
								<tr>
									<td rowSpan='3'>
										<Button
											className='botoncomp'
											id={'toggler' + componente.numero}
											onClick={() =>
												this.get_chart_data(componente.id_componente)
											}
										>
											{componente.numero}
										</Button>
										<Button
											className='botoncomp'
											onClick={() => this.activarEdicion(index)}
										>
											Editar
										</Button>
										<Button
											className='botoncomp'
											onClick={() => this.guardarproyeccioncomp(index)}
										>
											guardar
										</Button>
									</td>
									<td className='componentetitulo' rowSpan='3'>
										{componente.nombre}
									</td>
									<td>Avance Mensual</td>
									{this.state.mesesfor.map(mes => (
										<td className='avance'>
											{this.formatmoney(componente['M' + mes])}
										</td>
									))}
								</tr>
									...
									{this.state.mesesfor.map((mes, index_1) => (
										<td key={index_1}>
											<input
												className='inputproy'
												onBlur={evento =>
													this.capturaInputProyeccionExp(evento, mes, index)
												}
												placeholder={this.formatmoney(
													componente['MEXPT' + mes],
												)}
												disabled={
													this.state.activatorinput == index ? '' : 'disabled'
												}
											/>
										</td>
									))}
									...
									{this.state.mesesfor.map((mes, index_2) => (
										<td key={index_2}>
											<input
												className='inputproy'
												onBlur={evento =>
													this.capturaInputProyeccionVar(evento, mes, index)
												}
												placeholder={this.formatmoney(
													componente['MPROVAR' + mes],
												)}
												disabled={
													this.state.activatorinput == index ? '' : 'disabled'
												}
								...
								<tr>
									<td colSpan='15' className=''>
										...
										<UncontrolledCollapse
											toggler={'#toggler' + componente.numero}
										>
											<HighchartsReact
