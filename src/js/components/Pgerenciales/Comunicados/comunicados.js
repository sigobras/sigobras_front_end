...
class Comunicados extends Component {
    ...
    /////////FUNCIONES---------------------------------------/8///////////////////////////
    cargarObras() {
        axios.post(`${UrlServer}/obrasListaUser`,
            {
    ...
    async guardarComunicado() {
        ...
        await axios.post(`${UrlServer}/comunicados`,
    ...
    cargarObrasComunicados() {
        axios.post(`${UrlServer}/listaComunicados`,
            {
   ...
   async eliminarcomunicado(idcomunicados) {
    ...
    if (confirm("Al realizar esta acción no se podrá recuperar el comunicado, está seguro?")) {
        
        await  axios.post(`${UrlServer}/eliminarcomunicado`,
            {
    ...
    render() {
        ...
        return (
            ...
            <div>
                ...
                <form className="form">
                    <input name="comunicado" type="text" required autocomplete="off" onChange={event => this.selectinput(event)} />

                    <label className="lbl-nombre">

                    </label>
                </form>
                <input name="fecha_inicial" type="date" onChange={event => this.selectinput(event)} />
                <input name="fecha_final" type="date" onChange={event => this.selectinput(event)} />

                <button className="botoncomu" onClick={event => this.guardarComunicado()}>
                    GUARDAR
            </button>
                    ...
                    <table className="tablacomu table-hover">
                        ...
                        <tbody className="text-center">
                            {this.state.cajacomunicados.map((comunicado, index) =>
                                <tr key={index}>
                                    <td>
                                        <button onClick={() => this.eliminarcomunicado(comunicado.idcomunicados)}>Eliminar</button>
                                    </td>
                                    <td>
                                        {comunicado.codigo}
                                    </td>
                                    <td>
                                        {comunicado.fecha_inicial}
                                    </td>
                                    <td>
                                        {comunicado.fecha_final}
                                    </td>
                                    <td>
                                        {comunicado.texto_mensaje}
                                    </td>
                                </tr>
