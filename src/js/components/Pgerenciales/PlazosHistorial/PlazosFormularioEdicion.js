...
export default ({ data, recarga }) => {
	...
	async function enviarDatos(event) {
		...
		const clone = { ...FormularioDatos };
		clone.n_dias = calcular_dias(clone.fecha_inicio, clone.fecha_final);

		const res = await axios.put(`${UrlServer}/plazos`, clone);
	...
	function calcular_dias(fecha_inicio, fecha_final) {
		...
		const days = Math.round(Math.abs((firstDate - secondDate) / oneDay)) + 1;
		return days || 0;
	}
	...
	function asignarFechaMenor() {
		...
		console.log(
			'Formulario datos fecha inicial?',
