...
export default () => {
	...
	async function fetchDocumentosRecibidos() {
		const res = await axios.get(`${UrlServer}/gestiondocumentaria_recibidos`, {
			params: {
				id_acceso: sessionStorage.getItem('idacceso'),
				id_ficha: sessionStorage.getItem('idobra'),
			},
	...
	async function fetchDocumentosEnviados() {
		const res = await axios.get(`${UrlServer}/gestiondocumentaria_enviados`, {
			params: {
				id_acceso: sessionStorage.getItem('idacceso'),
			},
		});
	...
	const [DocumentoEnviadoSeleccionado, setDocumentoEnviadoSeleccionado] =
		useState(0);
	...
	async function actualizarVisto(gestiondocumentaria_mensajes_id) {
		try {
			const res = await axios.put(
				`${UrlServer}/gestiondocumentaria_receptores`,
				{
					id_ficha: sessionStorage.getItem('idobra'),
					id_acceso: sessionStorage.getItem('idacceso'),
					gestiondocumentaria_mensajes_id,
													...
													<tbody>
														{DocumentosEnviadosUsuarios.map((item2, i2) => [
															<tr key={i2}>
																...
																<td>
																	{item2.mensaje_visto ? (
																		<FaCheckCircle color='#0080ff' size={20} />
																	) : (
...
function RespuestasUsuariosEnviados({
	...
	const [DocumentosRecibidosRespuestas, setDocumentosRecibidosRespuestas] =
		useState([]);
