...
export default ({ recargar }) => {
	...
	async function fetchObras() {
		const res = await axios.post(`${UrlServer}/listaObrasByIdAcceso`, {
	...
	const [ObrasSeleccionadasProcesado, setObrasSeleccionadasProcesado] =
		useState([]);
	// cargos
	const [Cargos, setCargos] = useState([]);
	async function fetchCargos() {
		const res = await axios.get(`${UrlServer}/listaCargos`);
		setCargos(res.data);
	}
	...
	async function FormularioEnviar() {
		try {
			if (
				TipoArchivoSSeleccionado != 'SELECCIONE' &&
				...
				setLoaderShow(true);
				const obrasSeleccionadasProcesadas = [];
				...
				const res = await axios.post(
					`${UrlServer}/gestiondocumentaria_mensajes`,
					{
						mensaje: {
							emisor_id: sessionStorage.getItem('idacceso'),
							asunto: FormularioAsunto,
				...
				if (res.status == 200) {
					if (FormularioArchivoAdjunto) {
						...
						formData.append(
							'obra_codigo',
							sessionStorage.getItem('codigoObra'),
						);
						...
						formData.append(
							'tipoDocumento',
							TipoArchivoS.find(
								element => element.id == TipoArchivoSSeleccionado,
							).tipo,
						);
						formData.append('tipoDocumento_id', TipoArchivoSSeleccionado);
						const response = await axios.post(
							`${UrlServer}/gestiondocumentaria_mensajes_archivoAdjunto`,
							formData,
							{
								headers: {
	...
	async function fetchTipoArchivoS() {
		...
		const res = await axios.get(
			`${UrlServer}/gestiondocumentaria_archivosadjuntos_tipos`,
		);
		console.log(res.data);
		setTipoArchivoS(res.data);
	}
	const [TipoArchivoSSeleccionado, setTipoArchivoSSeleccionado] =
		useState('SELECCIONE');
			...
			<Modal isOpen={modal} toggle={toggle}>
				...
				<ModalBody>
					...
					<FormGroup row>
						...
						<Col sm={10}>
							<Input type='file' onChange={e => onChangeInputFile(e)} />
						</Col>
					</FormGroup>
				</ModalBody>
				<ModalFooter>
					{LoaderShow && <Spinner type='grow' color='primary' />}
					<Button color='primary' onClick={() => FormularioEnviar()}>
						GUARDAR
					</Button>{' '}
