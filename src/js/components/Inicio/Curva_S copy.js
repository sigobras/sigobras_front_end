...
function Curva_S({ id_ficha, codigo }) {
  ...
  async function solesToPorcentajeCurvaS(test) {
    ...
    cloneDataCurvaS.forEach((item, i) => {
      item.programado_monto = redondeo(
        (item.programado_monto / tempDataObra.costo_directo) * 100,
        2
      );
      item.ejecutado_monto = redondeo(
        (item.ejecutado_monto / tempDataObra.costo_directo) * 100,
        2
      );
      item.financiero_monto = redondeo(
        (item.financiero_monto / tempDataObra.g_total_presu) * 100,
        2
      );
    });
  ...
  function createDataChart(dataCurvaS) {
    ...
    dataCurvaS.forEach((item, i) => {
      if (item.tipo == "PERIODO") {
        var label =
          mesesShort[getMesfromDate(item.fecha_inicial) - 1] +
          " - " +
          getAnyofromDate(item.fecha_inicial);
        labels.push(label);
    ...
    for (let i = dataCurvaS.length - 1; i > 0; i--) {
      ...
      if (
        ejecutado_monto_break &&
        (item.ejecutado_monto == 0 || item.ejecutado_monto == null)
      ) {
        ejecutado.pop();
      } else {
      ...
      if (
        programado_monto_break &&
        (item.programado_monto == 0 || item.programado_monto == null)
      ) {
        programado.pop();
      } else {
  ...
  async function createDataChartPorcentaje(dataCurvaS) {
    ...
    dataCurvaS.forEach((item, i) => {
      if (item.tipo == "PERIODO") {
        ...
        programado_acumulado += item.programado_monto;
        programado.push(
          redondeo((programado_acumulado / tempDataObra.costo_directo) * 100, 2)
        );

        ejecutado_acumulado += item.ejecutado_monto;
        ejecutado.push(
          redondeo((ejecutado_acumulado / tempDataObra.costo_directo) * 100, 2)
        );

        financiero_acumulado += item.financiero_monto;
        financiero.push(
          redondeo((financiero_acumulado / tempDataObra.g_total_presu) * 100, 2)
        );
      }
    });
    //clean ejecutado
  ...
  function onChangeModalYears(value, name, i) {
    ...
    if (name == "anyo") {
      ...
      Dataclonado[i].programado_monto_propuesto = sumatoriaAnyo(
        value,
        "programado_monto"
      );
  ...
  const [
    FinancieroAcumuladoAnyoActual,
    setFinancieroAcumuladoAnyoActual,
  ] = useState(0);
  async function calcularSaldo(dataCurvaS) {
    ...
    clonDataObra.programado_acumulado =
      tempDataObra.costo_directo - clonDataObra.programado;
    clonDataObra.ejecutado_acumulado =
      tempDataObra.costo_directo - clonDataObra.ejecutado;
    clonDataObra.financiero_acumulado =
      tempDataObra.g_total_presu - clonDataObra.financiero;
        ...
        >
          {DataCurvaSTemp.length > 0
            ? [
                  ...
                  <div className="d-flex">
                    ...
                    >
                      <div>
                        ...
                        <div style={{ fontSize: "11px" }}>DELTA</div>
                      </div>
                    </Alert>
                    &nbsp;&nbsp;
                    <div>
                      ...
                      >
                        <div className="d-flex">
                          ...
                          <div style={{ fontWeight: 700 }}>
                            S/.
                            {Redondea(
                              PinDataMonto - FinancieroAcumuladoAnyoActual
                            )}
                          </div>
                        </div>
                      </Alert>
                ...
                >
                  <table key={2} className="table table-sm small">
                    <thead>
                      <tr>
                        <th>MES</th>
                        {DataCurvaSTemp.map(
                            ...
                            item.tipo == "PERIODO" && (
                              ...
                              >
                                <div className="d-flex">
                                  {(() => {
                                    ...
                                    return (
                                      myDate < today && (
                                        <div
                                          onClick={() =>
                                            updateEjecutado(item.fecha_inicial)
                                          }
                                        >
                                          <MdSystemUpdateAlt
                                            title={"ActualizarEjecutado"}
                                  ...
                                  {mesesShort[
                                    getMesfromDate(item.fecha_inicial) - 1
                                  ] +
                                    "-" +
                                    getAnyofromDate(item.fecha_inicial)}
                        ...
                        {DataCurvaSTemp.map(
                          (item, i) =>
                            item.tipo == "PERIODO" && (
                              <td key={i}>
                                {(item.ejecutado_monto == 0 ||
                                  anyoMes(item.fecha_inicial) ==
                                    anyoMesActual()) &&
                                EstadoInputProgramado == i ? (
                                  <div className="d-flex">
                                    <DebounceInput
                                      value={item.Programado_monto}
                                      debounceTimeout={300}
                                      onChange={(e) =>
                                        onChangeInputProgramado(
                                          e.target.value,
                                          i
                                        )
                                      }
            ...
            : [
                <img
                  ...
                  style={{ width: "300px" }}
                  src={
                    "https://www.jonmgomes.com/wp-content/uploads/2020/03/Pie_Chart_GIF_5_Seconds.gif?fbclid=IwAR34o4wzp3DOEtf1rKKG72-5bXCkXyuk7utSXYWmnB_k34XqnkPLPphmFio"
                  }
                />,
                (UsuarioData.cargo_nombre == "RESIDENTE" ||
                  UsuarioData.cargo_nombre == "EDITOR FINANCIERO") && (
            ...
            {FormularioOpcion == "total" && (
              <tbody>
                  ...
                  <tr key={i}>
                    <td>
                      <Input
                        ...
                        value={item.anyo}
                        onChange={(e) =>
                          onChangeModalYears(e.target.value, "anyo", i)
                        }
                        className="form-control"
                      >
                        <option disabled hidden>
                    ...
                    <td>
                      <DebounceInput
                        ...
                        debounceTimeout={300}
                        onChange={(e) =>
                          onChangeModalYears(e.target.value, "observacion", i)
                        }
                        type="text"
                        className="form-control"
                      />
                ...
                <tr>
                  <td colSpan={5}></td>
                  <td>
                    <Button color="danger" onClick={addYearModalYears}>
                      +
                    </Button>
                  </td>
                </tr>
