...
export default ({ recargar }) => {
	...
	async function guardarData(event) {
		...
		try {
			const res = await axios.post(`${UrlServer}/v1/analiticoCostos/obra`, {
	...
	const handleCreate = async inputValue => {
		...
		try {
			if (
				...
				const res = await axios.post(`${UrlServer}/v1/analiticoCostos`, {
					nombre: inputValue.toUpperCase(),
				});
				const clone = [...CostosNombres];
				const nuevaOpcion = { value: res.data.id, label: inputValue };
