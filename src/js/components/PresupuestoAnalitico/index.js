...
export default () => {
	...
	async function cargarPresupuestosAprobados() {
		const res = await axios.get(`${UrlServer}/v1/presupuestosAprobados`, {
			params: {
				id_ficha: sessionStorage.getItem('idobra'),
			},
		});
	...
	async function guardarPresupuesto(
		...
		const res = await axios.put(`${UrlServer}/v1/analitico/presupuesto`, {
			presupuesto_analitico_id,
			presupuestos_aprobados_id,
			monto: monto || 0,
		});
	...
	async function cargarCostosAnalitico() {
		const res = await axios.get(`${UrlServer}/v1/analiticoCostos`, {
			params: {
				id_ficha: sessionStorage.getItem('idobra'),
			},
		});
		setCostosAnalitico(res.data);
		const tempCostosCollapse = [];
	...
	async function eliminarCosto(id) {
		if (confirm('Esta seguro de eliminar el titulo?')) {
			const res = await axios.delete(`${UrlServer}/v1/analiticoCostos/${id}`);
			cargarCostosAnalitico();
		}
	}
	function RenderCostosData(item) {
		...
		if (Especificas.length > 0) {
			const properties = Object.keys(Especificas[0]).filter(
				element =>
	...
	async function cargarEspecificas() {
		const res = await axios.get(`${UrlServer}/v1/analitico/`, {
			params: {
				id_ficha: sessionStorage.getItem('idobra'),
	...
	async function cargarAnyosEjecutados() {
		const res = await axios.get(`${UrlServer}/v1/analitico/anyosEjecutados`, {
			params: {
				id_ficha: sessionStorage.getItem('idobra'),
				anyo: AnyoSeleccionado,
			},
...
function CustomAsyncSelect({ value, guardar }) {
	...
	async function clasificadorOptions(inputValue) {
		const res = await axios.get(`${UrlServer}/v1/clasificadorPresupuestario/`, {
			params: {
