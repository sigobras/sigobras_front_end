			...
			<Modal isOpen={modal} toggle={toggle}>
				...
				<Form onSubmit={enviarDatos}>
					<ModalBody>
						...
						<FormGroup>
							<Label for='exampleMontoAprobado'>
					...
					<tbody>
						{DatosCostosIndirectos.map((item, i) => (
							<tr>
								...
								<td>
									{Redondea(item.monto_expediente + item.monto_adicional)}
								</td>
								<tr>
									<FormularioEdicion
										data={item}
								...
								{(() =>
									Redondea(
										DatosCostosIndirectos.reduce(
											(accumulator, item) =>
												accumulator +
												item.monto_expediente +
												item.monto_adicional,
											0,
										),
									))()}
							</th>
...
function FormularioEdicion({ data, recarga }) {
	...
	async function enviarDatos(event) {
		event.preventDefault();
		// console.log(FormularioDatos);
		const clone = { ...FormularioEdicion };
		console.log('cloneedicion', clone);
		clone.monto_expediente = clone.monto_expediente.replace(/[^0-9\.-]+/g, '');
		clone.monto_adicional = clone.monto_adicional.replace(/[^0-9\.-]+/g, '');
		console.log('clone enviar', clone);
		const res = await axios.put(`${UrlServer}/costoIndirecto`, clone);
	...
	return (
		<td>
			<Button color='info' onClick={toggle} outline>
				<MdEdit />
			</Button>
			<Modal isOpen={modal} toggle={toggle}>
				<ModalHeader toggle={toggle}>Costos Indirectos</ModalHeader>
