...
function Redondea(
	...
	let decimalesPequenyos = decimales;
	if (Array.isArray(decimales)) {
		decimalesPequenyos = decimales[1];
	...
	if (Math.abs(x) < 0.01) {
		return Number.parseFloat(x)
			.CustomToFixed(decimalesPequenyos)
			.replace(/\d(?=(\d{3})+\.)/g, '$&,');
	} else {
		return Number.parseFloat(x)
...
function convertirFechaLetra(fecha) {
	if (fecha !== '') {
		const fechaLlega = new Date(fecha);

		const anio = fechaLlega.getFullYear();
		let mes = fechaLlega.getMonth() + 1;
		let dia = fechaLlega.getDate();
		if (mes <= 9) {
			mes = '0' + mes;
		}
		...
		const fechaLetra = new Date(formatearFecha).toLocaleDateString(
			'ES',
			config,
		);
...
function Extension(nombreArchivo) {
	...
	if (nombreArchivo) {
		const resultado = nombreArchivo.slice(
...
function getDaysBetweenDates(fechaInicio, fechaFinal) {
	const oneDay = 24 * 60 * 60 * 1000;
	const firstDate = new Date(fechaInicio);
	const secondDate = new Date(fechaFinal);
