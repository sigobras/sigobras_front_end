...
export default ({ partida, anyo, mes, imagenes_labels_id }) => {
	...
	async function eliminarImagen(tipo, id) {
		if (
			...
			try {
				const res = await axios.delete(`${UrlServer}/v1/partidasImagenes/`, {
					params: {
						tipo,
						id,
					},
...
const ImagenesLabelsListado = forwardRef(({ ImagenData }, ref) => {
	...
	async function cargarLabels() {
		const res = await axios.get(`${UrlServer}/v1/imagenesLabelsAsignadas`, {
			params: {
				id_partidaImagen:
...
function ComentariosInterfaz({ ImagenData }) {
	...
	async function cargarComentarios() {
		const res = await axios.get(`${UrlServer}/v1/imagenesComentarios`, {
			params: {
				id_partidaImagen:
					ImagenData.tipo == 'partidaImagen' ? ImagenData.id : null,
				id_AvanceActividades:
	...
	function handleInputChange(e) {
		const value = document.getElementById('comentario-input').innerHTML;
		const onlyText = value.replace(/<br>/g, '');
		if (onlyText) {
	...
	async function guardarComentario() {
		try {
			if (ComentarioText) {
				const res = await axios.post(`${UrlServer}/v1/imagenesComentarios`, {
		...
		>
			...
			{Comentarios.map((item, i) => (
				...
				>
					...
					>
						{`${item.cargo_nombre} ${item.usuario_nombre} ${item.usuario_apellido_paterno}`}
					</div>
