...
export default forwardRef(
	({ tipocomprobante_asignado, ModoEdicion, recargar, Permisos }, ref) => {
		...
		async function cargarComprobantesPago() {
			const res = await axios.get(`${UrlServer}/v1/comprobantesAsignados`, {
				params: {
		...
		async function agregarCommprobantesPago() {
			const res = await axios.post(`${UrlServer}/v1/comprobantesAsignados`, {
							...
							Permisos.fuentefinanciamiento_actualizar_comprobante != 1 ? (
								...
								<>
									<tr>
										...
										<td>
											{Permisos.fuentefinanciamiento_actualizar_comprobanteArchivo ==
												1 &&
												(!item.archivo || item.archivo == ' ') && (
													<FaUpload
														style={{ cursor: 'pointer' }}
											...
											{Permisos.fuentefinanciamiento_eliminar_comprobanteArchivo ==
												1 && (
												<FaReply
