...
export default ({
	...
	async function cargaTiposComprobantesPago() {
		const res = await axios.get(`${UrlServer}/v1/tiposComprobantesAsignados`, {
			params: {
				id_avancemensual,
			},
		});
	...
	async function asignarTipoComprobante() {
		...
		const tiposcomprobante_id = res.data.id;
									...
									<>
										<span className='d-flex'>
											<Input
												...
												onChange={e => {
													if (
														Permisos.fuentefinanciamiento_actualizar_tipocomprobante
													) {
											...
											{Permisos.fuentefinanciamiento_eliminar_tipocomprobante ==
												1 && (
												<FaRegTrashAlt
													style={{
														cursor: 'pointer',
