	...
	(
		...
		async function cargarEspecificas() {
			...
			if (Array.isArray(res.data)) {
				...
				const clone = [...CantidadEspecificas];
		...
		async function agregarEspecifica() {
			...
			const nextId = res.data ? res.data.id : maxId;
		...
		async function cargarCostosList() {
			const res = await axios.get(`${UrlServer}/v1/analiticoCostos/analitico`, {
				params: {
					id_ficha: sessionStorage.getItem('idobra'),
				},
			});
		...
		async function cargarVariacionesPim() {
			...
			const clone = [...CantidadPim];
			clone[Id_fuente] = res.data.length;
			setCantidadPim(clone);
		}
		async function agregarVariacionPim() {
			try {
				const nombreGenerado = 'PIM ' + (VariacionesPim.length + 1);
		...
		async function cargarTipoComprobantes() {
			const res = await axios.get(`${UrlServer}/v1/tiposcomprobantes`);
								...
								<>
											...
											{EstadoEdicion != 'especifica_' + item.id ? (
												<span
													onClick={() => {
														if (
															ModoEdicion &&
															Permisos.fuentefinanciamiento_editar_especifica ==
																1
														) {
										...
										{EstadoEdicion != 'especifica_' + item.id && (
											<th className='whiteThem-table-sticky2'>
												...
												>
													...
													{ModoEdicion &&
														Permisos.fuentefinanciamiento_eliminar_especifica ==
															1 && (
															<FaRegTrashAlt
									...
									{Costos.filter(
										...
										<tr key={i2}>
													...
													<>
														<span
															onClick={() => {
																if (
																	ModoEdicion &&
																	Permisos.fuentefinanciamiento_editar_titulo ==
																		1
																) {
														...
														{ModoEdicion &&
															Permisos.fuentefinanciamiento_eliminar_titulo ==
																1 && (
																<span
																	style={{
																		cursor: 'pointer',
											...
											<td colSpan={1 + VariacionesPim.length} />
...
function CustomAsyncSelect({ value, guardar }) {
	...
	function handleInputChange(input) {
		setFlagCambios(true);
		const inputData = input.label.split('-');
		setValue({
