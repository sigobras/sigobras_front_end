...
const Mdiario = () => {
	...
	async function fetchEstadoObra() {
		const res = await axios.post(`${UrlServer}/getEstadoObra`, {
			id_ficha: sessionStorage.getItem('idobra'),
		});
				...
				>
					...
					<tbody>
						{Partidas.map(item => [
							...
							>
								<td>
									{Permisos.ver_chatpartidas == 1 && item.tipo == 'partida' && (
										<PartidasChat
