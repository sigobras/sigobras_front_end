...
class TblResumenCompDrag extends Component {
	...
	componentDidMount() {
		...
		axios
			.post(`${UrlServer}/getmaterialesResumenEjecucionRealCodigos`, {
				id_ficha: IdObra,
				tipo: tipoRecurso,
			})
			.then(res => {
		...
		axios
			.post(`${UrlServer}/getmaterialesResumenEjecucionRealSinCodigo`, {
				id_ficha: IdObra,
				tipo: tipoRecurso,
			})
			.then(res => {
	...
	onDrop(ev, cat, codigo, indexC, icodi, idDocumento) {
		...
		if (cat === 'completado') {
			...
			const NuevaDataEliminada = [];

			const FitraDrecrip = DataRecursoAgrupadoApi.filter(descrip => {
			...
			const CantidadSuma =
				DataCodigosAgrupado[indexC].codigos[icodi].cantidad + 1;
			...
			axios
				.post(`${UrlServer}/postrecursosEjecucionreal`, {
					tipo: 'codigo',
					data: [[IdObra, tipoRecurso, id, codigo, idDocumento]],
				})
				.then(res => {
	...
	render() {
		...
		const externalCloseBtn = (
			<button
				...
				onClick={this.ModalVermasRecursoAgrupado.bind(this, '', '', '', '')}
			>
				&times;
			</button>
		);
		return (
			<div>
				<Row>
					<Col sm='7'>
						<div
							...
							onDrop={e => {
								this.onDrop(e, 'ejecucion', '', 'in', 'in2', 'idDocumento');
							}}
						>
									...
									{DataRecursoAgrupadoApi !== undefined ? (
										DataRecursoAgrupadoApi.map((ReqLista, IndexRL) => (
											<tr
												key={IndexRL}
												onDragStart={e =>
													this.onDragStart(e, ReqLista.descripcion)
												}
												draggable
												className='grabbable'
											>
										...
										<tr>
											<td colSpan='11'>AUN NO HAY DATOS</td>
										</tr>
									)}
								</tbody>
							</table>
									...
									<div className='p-1 d-flex flex-wrap'>
										{tipoDoc.codigos.map((codigo, icodi) => (
											<div
												...
												key={icodi}
												onDragOver={
													codigo.bloqueado !== 1 ? e => this.onDragOver(e) : ''
												}
												onDrop={e => {
													this.onDrop(
														e,
												...
												onClick={this.ModalVermasRecursoAgrupado.bind(
													this,
													codigo.codigo,
													tipoDoc.idDocumento,
													tipoDoc.tipoDocumento,
				...
				<Modal
					isOpen={this.state.modalMostrarMasCodigo}
					toggle={this.ModalVermasRecursoAgrupado.bind(this, '', '', '', '')}
					external={externalCloseBtn}
					size='lg'
				>
					<DataSegunCodigoSelect
						DataConsumir={Object.assign(
