...
class DataSegunCodigoSelect extends Component {
	...
	componentDidMount() {
		...
		axios
			...
			.then(res => {
				...
				const totalParcial = Object.values(res.data.recursos).reduce(
					(anterior, actual) => {
						return (
	...
	GuardaOrdenDeCompra(e) {
		...
		const funcion = data => [IdObra, data.descripcion];

		const dataRecorre = recursos.map(funcion);

		const razonSocial = e.target[2].value;
		const RUC = e.target[3].value;
		const fecha = e.target[5].value;
		const SIAF = e.target[6].value;
		const NCP = e.target[7].value;
		const DataNuevo = {
			razonSocial,
			RUC,
			fecha,
