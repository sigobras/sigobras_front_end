...
class ListaMateriales extends Component {
	...
	componentDidMount() {
		...
		axios
			...
			.then(res => {
				...
				const partidas = res.data[0].partidas;
		...
		axios
			...
			.then(res => {
				...
				const CategoriasIconos = res.data;
	...
	UpdatePrioridadIcono(idPartida, id_icono, index) {
		...
		axios
			...
			.then(res => {
				...
				const partidas = this.state.DataPartidas;
				let CategoriasIconos = res.data.nombre;
	...
	activaEditable(index, CantPrecio, selectTipoDocumento) {
		...
		if (CantPrecio === null) {
			const Idselect = this.state.DataGuardarInput.data[0][4] || '';
			...
			if (this.state.DataGuardarInput.data[0][3] !== '') {
				...
				axios
					...
					.then(res => {
						...
						const DataRecursosListaApi = this.state.DataRecursosListaApi;

						let EncuentraTipoOrdenCompra =
							DataRecursosListaApi[index].tipodocumentoadquisicion_nombre;
						if (Idselect !== '') {
							// console.log("this.state.DataGuardarInput ", Number(this.state.DataGuardarInput.data[0][4]))

							const EncuentraTipoOrdenC =
	...
	GuardaNuevaOC(descripcion, index) {
		...
		const dataFiltro = DataRecursosListaApi.filter(DataRecurso => {
			return DataRecurso.descripcion === descripcion;
		});

		const dataEncontrado = dataFiltro[0];
		...
		if (dataEncontrado) {
			...
			const dataFiltroTipoOrdenCompra = DataTipoDocAdquisicionApi.filter(
				DataRecurso => {
					return (
	...
	agregaRecursoNuevo(orden) {
		const DataListaRecursos = this.state.DataRecursosListaApi;
	...
	GuardaNuevoRecursoApi() {
		...
		const DataListaRecursos = DataRecursosListaApi[0];
		...
		const dataEncontrado = dataFiltroTipoOrdenCompra[0];
	...
	render() {
		...
		let DatosPartidasFiltrado = DataPartidas;
		let BuscaPartida = this.state.BuscaPartida;
