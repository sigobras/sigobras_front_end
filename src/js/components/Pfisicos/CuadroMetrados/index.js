...
export default () => {
	...
	async function fetchAnyos() {
		const request = await axios.post(`${UrlServer}/getHistorialAnyos2`, {
			id_ficha: sessionStorage.getItem('idobra'),
		});
		setAnyos(request.data);
		const ultimoAnyo = request.data[request.data.length - 1].anyo;
	...
	async function fetchMeses() {
		const res = await axios.post(`${UrlServer}/getHistorialMeses2`, {
			id_ficha: sessionStorage.getItem('idobra'),
	...
	async function fetchComponentes() {
		const res = await axios.post(`${UrlServer}/getHistorialComponentes2`, {
			id_ficha: sessionStorage.getItem('idobra'),
			anyo: AnyoSeleccionado,
			mes: MesSeleccionado,
		});
	...
	async function fetchAvances() {
		const res = await axios.get(`${UrlServer}/v1/avance/cuadroMetrados`, {
			params: {
				anyo: AnyoSeleccionado,
				mes: MesSeleccionado,
	...
	async function fetchAvancesResumen() {
		const res = await axios.get(
			`${UrlServer}/v1/avance/cuadroMetradosResumen`,
			{
				params: {
					anyo: AnyoSeleccionado,
					mes: MesSeleccionado,
	...
	function renderMesesCabezera() {
		...
		if (Avances && Avances[0]) {
			const properties = Object.keys(Avances[0]).filter(element =>
	...
	function renderRecursosNombres() {
		const render = [];
		if (Avances.length) {
			const item = Avances[0];
			const properties = Object.keys(item).filter(element =>
				element.startsWith('recurso_'),
			);
			render.push(
						...
						<tbody>
							{Avances.map((item, i) => (
								<tr key={i}>
									...
									>
										<div className='cut-text '>{item.descripcion}</div>
									</td>
