...
function HistorialObservaciones() {
	const [DificultadComentariosNoVistos, setDificultadComentariosNoVistos] =
		useState([]);
	...
	async function getCantidadComentarios(tipo) {
		...
		const req_comentariosNoVistos = await axios.post(
	...
	function onchangeDificultades(value, name) {
		const Dataclonado = { ...newDificultad };
		...
		if (
			...
			let dias = Dataclonado.duracion;
			if (Dataclonado.duracion_tipo == 'horas') {
				dias = Dataclonado.duracion / 8;
			}
			let fecha_inicio = Dataclonado.fecha_inicio;
