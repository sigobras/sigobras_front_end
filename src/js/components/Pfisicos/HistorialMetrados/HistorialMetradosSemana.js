...
export default ({ Semana, UsuarioData }) => {
	...
	async function fetchSemanaFechas() {
		setLoading(true);
		const request = await axios.post(`${UrlServer}/getHistorialSemanalFechas`, {
	...
	const [SpinnerSemanaFechaSeleccionada, setSpinnerSemanaFechaSeleccionada] =
		useState(false);
	...
	async function fetchSemanasComponentes() {
		...
		const semanasComponentes = [...request.data];
	...
	async function getHistorialSemanalDias(id_componente, fecha) {
		const request = await axios.post(`${UrlServer}/getHistorialSemanalDias`, {
			id_componente,
			fecha,
		});
		return request.data;
	...
	const [InputAvanceActividadIndex, setInputAvanceActividadIndex] =
		useState(-1);
	const [InputAvanceActividadData, setInputAvanceActividadData] =
		useState(null);
