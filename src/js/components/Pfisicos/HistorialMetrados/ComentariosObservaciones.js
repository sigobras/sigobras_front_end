...
function Comentarios({ dificultades_id }) {
	...
	async function fetchComentario() {
		const request = await axios.post(
			`${UrlServer}/getDificultadesComentarios`,
			{
				dificultades_id,
			},
		);
	...
	async function saveComentario() {
		const request = await axios.post(
			`${UrlServer}/postDificultadesComentarios`,
			{
	...
	function socketIni() {
		console.log(
			'escuchando',
			'dificultades_comentarios_get-' + dificultades_id,
		);
		...
		<ModalBody style={{ maxHeight: 'calc(100vh - 210px)', overflowY: 'auto' }}>
			...
			<div className='input-group mt-2'>
				...
				<div className='input-group-append'>
					<Button color='primary' onClick={() => saveComentario()}>
						Guardar
					</Button>
				</div>
			</div>
