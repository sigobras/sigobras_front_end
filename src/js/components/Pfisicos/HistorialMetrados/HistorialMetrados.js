...
export default () => {
	...
	async function fetchMeses() {
		const request = await axios.post(`${UrlServer}/getHistorialMeses2`, {
			id_ficha: sessionStorage.getItem('idobra'),
	...
	async function fetchComponentes() {
		const request = await axios.post(`${UrlServer}/getHistorialComponentes2`, {
			id_ficha: sessionStorage.getItem('idobra'),
			anyo: AnyoSeleccionado,
			mes: MesSeleccionado,
		});
	...
	async function fetchSemanas() {
		const res = await axios.post(`${UrlServer}/getHistorialSemanas`, {
			id_ficha: sessionStorage.getItem('idobra'),
			anyo: AnyoSeleccionado,
			mes: MesSeleccionado,
		});
	...
	return (
		<div>
			<Nav tabs>
				...
				<NavItem>
					<NavLink
						className={classnames({ active: MesSeleccionado == 0 })}
						onClick={() => {
							setMesSeleccionado(0);
						}}
					>
