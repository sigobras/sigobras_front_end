...
export default forwardRef(({ id_actividad }, ref) => {
    ...
    async function fectchAvanceActividad() {
        ...
        console.log("setAvanceActividad", request.data);
        setAvanceActividad(request.data)
        setAvanceActividadPorcentaje(Redondea1(request.data.avance_metrado / request.data.metrado * 100))
    }
        ...
        <div>
            ...
            >
                <div
                    style={{
                        ...
                        height: '100%',
                        background: AvanceActividadPorcentaje > 85 ? '#a4fb01'
                            : AvanceActividadPorcentaje > 30 ? '#0080ff'
                                : '#ff2e00',
            ...
            <div className="clearfix">
                <span className="float-left text-info">
                    Saldo: {Redondea(AvanceActividad.metrado - AvanceActividad.avance_metrado)}
                </span>
                <span className="float-right text-info">
                    S/.{Redondea(AvanceActividad.presupuesto - AvanceActividad.avance_soles)}
                </span>
            </div>
        </div>
